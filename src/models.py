...
class CustomTransformerEncoder(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        for layer in self.layers:
            output, attention_weights = layer(output, src_mask=src_mask, src_key_padding_mask=src_key_padding_mask)
...
class MISA(nn.Module):
    def __init__(self, config):
        ...
        #encoder_layer = nn.TransformerEncoderLayer(d_model=self.config.hidden_size, nhead=2) # 출력 Tuple 6
        #self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=1)
        
        encoder_layer = CustomTransformerEncoderLayer(d_model=self.config.hidden_size, nhead=2) # 출력 Tuple 2
        self.transformer_encoder = CustomTransformerEncoder(encoder_layer, num_layers=1)

        # 입력 데이터
        # src = torch.rand(32, 10, d_model)  # [seq_length, batch_size, d_model]

        # Encoder를 통과시키기
        #output, attn_weights = transformer_encoder(src)

        # attn_weights는 [batch_size, nhead, seq_length, seq_length] 형태의 텐서입니다.
        # 이 텐서는 각 시퀀스 위치에서 다른 시퀀스 위치로의 주의 정도를 나타냅니다.
    ...
    '''
    # 시각화 및 저장을 위한 함수 정의
    def visualize_and_save_attention(self, attention_weights, layer_idx, head_idx, save_path):
        # attention_weights는 [batch_size, num_heads, seq_length, seq_length] 형태로 가정
        # 특정 layer와 head의 attention을 시각화합니다.
        attention = attention_weights[layer_idx][head_idx].detach().numpy()
        ...
        plt.savefig(f"{save_path}/layer_{layer_idx}_head_{head_idx}_attention.png")
        plt.close()
    '''
    '''
    # 시각화 및 저장을 위한 함수 정의
    def visualize_and_save_attention(self, attention_weights, layer_idx, head_idx, save_path):
        ...
        # 시각화
        plt.figure(figsize=(10, 8))
        # 'Purples' 또는 'BuPu'로 변경하여 색상 스키마를 조정합니다.
    ...
    def visualize_and_save_attention_weights(self, attention_weights, save_path, batch_index=0):
        # attention_weights는 [batch_size, num_heads, seq_length, seq_length] 형태로 가정
        # 특정 layer와 head의 attention을 시각화합니다.
        attention_matrix = attention_weights[batch_index].detach().cpu().numpy()
        ...
        # 파일명 중복을 피하기 위한 인덱스 추가
        base_filename = f"attention_weights"
        extension = ".png"
        file_number = 1
        while os.path.exists(os.path.join(save_path, f"{base_filename}_{file_number:03d}{extension}")):
            file_number += 1
        final_filename = f"{base_filename}_{file_number:03d}{extension}"

        # 저장
        plt.savefig(os.path.join(save_path, final_filename))
        plt.close()
    ...
    def alignment(self, sentences, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        ...
        #print('batch_size') # 32개
        #print(batch_size)
        attention_weights = None  # attention weights 초기화
        ...
        if self.config.use_bert:
            ...
            self_attention_weights = bert_output[-1]
            ...
            #print(utterance_text.shape) # torch.Size([32, 128*6])
        ...
        attention_weights = h[1] # weight
        
        self.visualize_and_save_attention_weights(attention_weights[0], './attention_weight', batch_index=0)
        
        h = torch.cat((h[0][0], h[0][1], h[0][2], h[0][3], h[0][4], h[0][5]), dim=1)
    ...
    def shared_private(self, utterance_t, utterance_v, utterance_a):
        ...
        #print('Test Latent Space')
        emb = torch.cat((self.utt_private_t, self.utt_private_v, self.utt_private_a, 
                  self.utt_shared_t, self.utt_shared_v, self.utt_shared_a), dim=0) # dim=0인 경우
        #print(emb.size())
        self.count += 1
        np.save('./latent_space/' + str(self.count), emb.cpu().detach().numpy()) # x_save.npy  
        

    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        batch_size = lengths.size(0)
        o, _ = self.alignment(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask)
        return o
